...
class TinyChessTransformer(nn.Module):
    ...
    def forward(self, x, training: bool):
        ...
        policy_logits = p.permute(0, 3, 2, 1).flatten(1)
...
def hl_gauss_loss(logits, target_indices, num_bins=128, sigma=2.0): # LOWER THIS
    """
    logits: [Batch, 128] - The raw output from the model (before Softmax)
    target_indices: [Batch] - The correct bin index (0-127) as LongTensor
    sigma: Controls how 'wide' the acceptable answer is. 
           sigma=0 -> Hard CrossEntropy (One-Hot)
           sigma=2.0 -> Standard smoothing for ~128 bins
    """
    
    support = torch.arange(num_bins, device=logits.device, dtype=torch.float32)
    
    # 2. Expand targets to allow broadcasting: [Batch, 1]
    target_expanded = target_indices.unsqueeze(1).float()
    
    # Gaussian dist
    soft_targets = torch.exp(-(support - target_expanded)**2 / (2 * sigma**2))
    
    # norm
    soft_targets = soft_targets / soft_targets.sum(dim=1, keepdim=True)
    
    return torch.nn.functional.cross_entropy(logits, soft_targets)

if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = TinyChessTransformer().to(device).to(torch.bfloat16)
    
    # Dummy Input: 111 channels, test inference speed
    dummy_input = torch.rand((8, 111, 8, 8), dtype=torch.bfloat16).to(device)
    with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
        policy, val= model(dummy_input, training=True)
        loss = policy.mean()
